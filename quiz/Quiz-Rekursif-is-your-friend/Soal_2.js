...
function changeXRecursive(data, jenis) {
  ...
  let ganjil_genap = parseInt(indeks) % 2 === 0;
  
  if (jenis === "ganjil" && ganjil_genap) {
    return "x" + changeXRecursive(sisa, jenis);
  } else if (jenis === "genap" && !ganjil_genap) {
    return "x" + changeXRecursive(sisa, jenis);
  } else {
    return indeks + changeXRecursive(sisa, jenis);
  }
}
