...
function economyChangeSummary(tradeActivity) {
  ...
  for (let i = 0; i < tradeActivity.length; i++) {
    for (let j = 0; j < tradeActivity[i].length; j++) {
      ...
      for (let k = 0; k < tradeActivity[i][j].length; k++) {
        if (tradeActivity[i][j][k].match(/[a-z\ ]/gi)) {
          nama += tradeActivity[i][j][k];
        } else if (tradeActivity[i][j][k].match(/[+-]/gi)) {
          plusminus += tradeActivity[i][j][k];
        } else if (tradeActivity[i][j][k].match(/[0-9]/gi)) {
          persen += tradeActivity[i][j][k];
        }
      }
      hasil.push(nama);
      hasil.push(plusminus);
      hasil.push(persen);
      arr_nama.push(hasil);
    }
  }
  ...
  let Owner = "";
  let depo = 0;
  for (let i = 0; i < arr_nama.length; i++) {
    if (arr_nama[i][0] === "Jeff Bezos") {
      Owner = "Amazon";
      if (arr_nama[i][1] === "+") {
        duitJeff = duitJeff + (duitJeff * (Number(arr_nama[i][2]) / 100));
      }
      if (arr_nama[i][1] === "-") {
        duitJeff = duitJeff - (duitJeff * (Number(arr_nama[i][2]) / 100));
      }
    } else if (arr_nama[i][0] === "Larry Page") {
      Owner = "Google";
      if (arr_nama[i][1] === "+") {
        duitLarry = duitLarry + (duitLarry * (Number(arr_nama[i][2]) / 100));
      }
      if (arr_nama[i][1] === "-") {
        duitLarry = duitLarry - (duitLarry * (Number(arr_nama[i][2]) / 100));
      }
    } else if (arr_nama[i][0] === "Jack Ma") {
      Owner = "Aibaba";
      if (arr_nama[i][1] === "+") {
        duitJack = duitJack + (duitJack * (Number(arr_nama[i][2]) / 100));
      }
      if (arr_nama[i][1] === "-") {
        duitJack = duitJack - (duitJack * (Number(arr_nama[i][2]) / 100));
      }
    ...
    objek = {
      name: arr_nama[i][0],
      deposit: arr_nama[i][0] === "Jeff Bezos" ? duitJeff : arr_nama[i][0] === "Larry Page" ? duitLarry : duitJack,
      owner: Owner
    }
