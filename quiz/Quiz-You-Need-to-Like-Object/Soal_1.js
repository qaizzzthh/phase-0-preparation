...
function ganjilGenapAsianGames(date, data) {
  ...
  for (let i = 0; i < data.length; i++) {
    let plat = [];
    plat.push(data[i].plat);
    plat_kendaraan.push(plat[0].split(" "));
  }

  if (date > 0 && date < 32) {
    if (date % 2 === 0) {
      let jumlah = 0;
      for (let i = 0; i < plat_kendaraan.length; i++) {
        if (Number(plat_kendaraan[i][1]) % 2 === 0) {
          jumlah++;
        }
      }
      return jumlah;
      ...
      let jumlah = 0;
      for (let i = 0; i < plat_kendaraan.length; i++) {
        if (Number(plat_kendaraan[i][1]) % 2 === 1) {
          jumlah++;
        }
      }
      return jumlah;
